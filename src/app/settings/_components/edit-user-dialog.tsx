...
export function EditUserDialog({ isOpen, onClose, user, currentUserProfile }: EditUserDialogProps) {
  ...
  const [selectedGrades, setSelectedGrades] = useState<string[]>(
    (user as any).gradeIds || ((user as any).gradeId ? [(user as any).gradeId] : [])
  );
  ...
  useEffect(() => {
    ...
    setSelectedGrades((user as any).gradeIds || ((user as any).gradeId ? [(user as any).gradeId] : []));
  }, [user]);
      ...
      <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <form action={dispatch} className="space-y-4 pt-2">
            ...
            {!isAdmin && <input type="hidden" name="role" value={user.role} />}
